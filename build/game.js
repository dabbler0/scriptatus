...
(function() {
  var ANGULAR_FRICTION, ARCHER_AI, ARCHETYPES, ARROW_LENGTH, ARROW_VELOCITY, AiWorker, Archer, BOARD_HEIGHT, BOARD_WIDTH, BOWSTRING_COLOR, BOW_COLOR, BOW_HEIGHT, BOW_THICKNESS, Bullet, Character, DAGGER_COLOR, DAGGER_LENGTH, DAGGER_LIFETIME, DAGGER_VELOCITY, DEFAULT_COLORS, DUMBO, FRAME_RATE, FRICTION, KNIGHT_AI, Knight, MAGE_AI, Mage, NECK_HEIGHT, Particle, ROGUE_AI, RenderContext, Rogue, SHIELD_RATIO, SPELL_RADIUS, Spell, TORSO_PROPORTION, Vector, WALKING_PERIOD, WALKING_RATIO, WALL_HEIGHT, Wall, ace_editor, character_templates, create_ai_from_template, currently_editing, edit_screen, fn, i, index_positions, infill_object, instantiate_character, j, key_codes, lose_screen, main_menu, play_game, sign, win_screen, wrap_angle,
    ...
    AiWorker.prototype.tick = function(info) {
      if (this.ready) {
        this.worker.postMessage({
          characters: info.characters.filter(function(x) {
            return x.health > 0;
          }).map((function(_this) {
            return function(x) {
  ...
  Mage = (function(superClass) {
    ...
    Mage.prototype.cast = function(position) {
      if (this.spell.burning || this.health <= 0) {
        return;
      }
      this.casting = true;
  ...
  Bullet = (function() {
    function Bullet(pos1, velocity, height, length, lifetime, color, damage1) {
      this.pos = pos1;
  ...
  index_positions = [new Vector(50, 50), new Vector(50, 100), new Vector(100, 50), new Vector(100, 100)];

  instantiate_character = function(template, index) {
    switch (template["class"]) {
      case 'Mage':
        return new Mage(50, 10, index_positions[index].clone(), true, template.ai);
      case 'Knight':
        return new Knight(50, 10, index_positions[index].clone(), true, template.ai);
      case 'Rogue':
        return new Rogue(50, 10, index_positions[index].clone(), true, template.ai);
      case 'Archer':
        return new Archer(50, 10, index_positions[index].clone(), true, template.ai);
    }
  };
  ...
  play_game = function() {
    var bullets, canvas, characters, contexts, ctx, desired_pos, dirt_asset, fn, grass_asset, grass_spots, i, j, k, keysdown, l, len, main_character, moving_target, new_walls, none_canvas, none_ctx, quit_canvas, quit_ctx, ref, render_context, results, should_continue_tick, spells, tick, tile_height, tile_width, translate_vector, wall, walls;
    ...
    characters = characters.concat([new Mage(50, 10, new Vector(BOARD_WIDTH - 50, BOARD_HEIGHT - 50), false, MAGE_AI), new Rogue(50, 10, new Vector(BOARD_WIDTH - 100, BOARD_HEIGHT - 50), false, ROGUE_AI), new Knight(50, 10, new Vector(BOARD_WIDTH - 100, BOARD_HEIGHT - 100), false, KNIGHT_AI), new Archer(50, 10, new Vector(BOARD_WIDTH - 50, BOARD_HEIGHT - 100), false, ARCHER_AI)]);
    ...
    grass_spots = (function() {
      results = [];
      for (var k = 1, ref = Math.floor(Math.random() * 20 + 10); 1 <= ref ? k <= ref : k >= ref; 1 <= ref ? k++ : k--){ results.push(k); }
      return results;
    ...
    fn = function(i) {
      return document.getElementById("char-" + i).addEventListener('click', function() {
        if (main_character) {
          main_character.player_controlled = false;
        }
        main_character = characters[i - 1];
        return main_character.player_controlled = true;
      });
    };
    for (i = l = 1; l <= 4; i = ++l) {
      fn(i);
    ...
    tick = function() {
      var bottom_intersect, bullet, character, context, desired_dir, distances, entities, entity, i1, left_intersect, len1, len10, len11, len2, len3, len4, len5, len6, len7, len8, len9, m, min_dist, n, new_bullets, normalized_delta, prototype_vector, q, r, ref1, ref2, result, right_intersect, spell, spot, t, target, top_intersect, u, v, w, y, z;
      if (desired_pos.x < 50) {
      ...
      for (i = m = 0, len1 = contexts.length; m < len1; i = ++m) {
        context = contexts[i];
      ...
      for (n = 0, len2 = grass_spots.length; n < len2; n++) {
        spot = grass_spots[n];
        ctx.drawImage(grass_asset, spot.x, spot.y);
      }
      for (q = 0, len3 = spells.length; q < len3; q++) {
        spell = spells[q];
        spell.tick();
        spell.render(render_context);
      }
      for (r = 0, len4 = characters.length; r < len4; r++) {
        character = characters[r];
        if (!(character.health > 0)) {
          continue;
        }
        for (t = 0, len5 = spells.length; t < len5; t++) {
          spell = spells[t];
        ...
        if (character.striking_sideways) {
          for (u = 0, len6 = characters.length; u < len6; u++) {
            target = characters[u];
            if (target !== character && character.health > 0) {
              if (character.pos.minus(target.pos).magnitude() < character.radius * 2.5 + target.radius && Math.abs(wrap_angle(target.pos.minus(character.pos).dir() - character.dir)) < Math.PI / 2) {
                target.damage(3);
              }
            }
          }
        ...
        for (v = 0, len7 = walls.length; v < len7; v++) {
          wall = walls[v];
      ...
      for (w = 0, len8 = entities.length; w < len8; w++) {
        entity = entities[w];
      ...
      new_bullets = [];
      for (y = 0, len9 = bullets.length; y < len9; y++) {
        bullet = bullets[y];
        bullet.tick();
        bullet.render(render_context);
        for (z = 0, len10 = characters.length; z < len10; z++) {
          character = characters[z];
          if (character.health > 0) {
        ...
        for (i1 = 0, len11 = walls.length; i1 < len11; i1++) {
          wall = walls[i1];
  ...
  ace_editor.on('change', function() {
    return character_templates[currently_editing].ai = ace_editor.getValue();
  });

  edit_screen = function() {
    var canvas, ctx, i, j, len, results, template;
    document.getElementById('win-screen').style.display = 'none';
    ...
    for (i = j = 0, len = character_templates.length; j < len; i = ++j) {
      ...
      ctx = canvas.getContext('2d');
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      results.push(ARCHETYPES[template["class"]].render(new RenderContext(canvas, ctx)));
    }
    return results;
  };

  fn = function(i) {
    return document.getElementById("edit-tab-" + (i + 1)).addEventListener('click', function(x) {
      currently_editing = i;
      return ace_editor.setValue(character_templates[i].ai, -1);
    });
  };
  for (i = j = 0; j < 4; i = ++j) {
    fn(i);
